...
- **`apiBaseUrl` 已删除**：出站媒体遇到相对 URL 时 fail-closed 报错；由 Host 投影的出站媒体地址需要是绝对 URL，相对 URL 媒体发送被推迟（不再可配置 base URL）。
