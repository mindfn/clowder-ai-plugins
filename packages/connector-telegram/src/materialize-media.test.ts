...
test('materializeMedia stops at the platform limit and removes the partial file', async () => {
  const before = new Set((await readdir(tmpdir())).filter(name => name.startsWith('clowder-telegram-outbound-')));
  await assert.rejects(materializeMedia(chunks(), 'too-large.bin', 11), /platform limit/);
  const after = (await readdir(tmpdir())).filter(name => name.startsWith('clowder-telegram-outbound-'));
