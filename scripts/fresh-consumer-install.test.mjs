...
test('packed public packages install and import in a fresh npm consumer', async () => {
  ...
  try {
    ...
    assert.equal(sdkPackage.version, '0.2.0-beta.4');
    assert.equal(sdkPackage.dependencies['@clowder-ai/plugin-contract'], '0.1.0-beta.21');
    assert.equal(
    ...
    assert.equal(weixinMpPackage.dependencies['@clowder-ai/plugin-sdk'], '0.2.0-beta.4');
    assert.doesNotMatch(JSON.stringify(weixinMpPackage), /"workspace:/u);
    assert.equal(wechatReaderPackage.version, '0.1.0-alpha.1');
    assert.equal(wechatReaderPackage.dependencies['@clowder-ai/plugin-sdk'], '0.2.0-beta.4');
    ...
    assert.equal(enterprisePackage.dependencies['@clowder-ai/plugin-sdk'], '0.2.0-beta.4');
