...
**状态**：D34（revision 11 body），R9 P1 已修 + terra 两轮扫描全吸收（R45：envelope 全传播 + T-D totality；R46：response lane 拆 accept/close；R47/R48：T-L 谓词精确化；R49：R10 三处 derived projection 同步——five-axis 四终态 / checklist T-A…T-L / conformance 计数 5+4）——pending 复扫——先扫后投。
...
## Status and request — revision 11: your R10 exact-body audit absorbed — every derived projection now states the four terminal categories
...
|---|---|
| complete request/result/notification/**full error** object | raw frame/object grammar = W0; outer/fixed leaves = W1–W3; each row payload = its H/M/I join in the row projection. Errors have no free string branch beyond W1 because messages/data are W3 finite literals. **The outer error lattice is complete via the disposition table as the projection of the six-way state partition: every class carries exactly one of the **four terminal categories** — legal-Notification dispatch with no response (T-J), valid-correlated-Response settle with no response (T-L), a closed byte-proved error response, or connection close — no Notification or Response candidate is ever answered, and close/accept classes (T-J/T-L and every close row) are outside the public union by construction.** Rows reaching reserved leaves are called reserved, not closed. |
...
- [x] **every pre-dispatch rejection class has exactly one deterministic disposition** — your R6 invariant adopted verbatim; disposition table **T-A…T-L** as the projection of the six-way state partition (your R7 state-model requirement), including non-object frame roots (T-A), the response lane split — malformed/unmatched candidates close (T-H), a valid correlated Response settles silently (**T-L**) — in-flight id collisions as their own close-class (T-I), and the Notification split (T-J accept / T-K close — **once classified as a Notification, no Response is ever emitted**, §4.1); strict-profile id route: detectable-but-profile-invalid → close, no response; close/accept classes (T-J/T-L + all close rows) excluded from the public union/proofs by construction
...
- [ ] **fresh review on revision-11 exact live bytes** (raw API-string SHA-256/counts reported in delivery comment)
