...
**状态**：D33（revision 10 body），R9 P1 已修 + terra 两轮扫描全吸收（R45：envelope 全传播 + T-D totality；R46：response lane 拆 accept/close；R47/R48：T-L per-row/cross-frame 谓词落表 + 四处 prose 同步）——pending 复扫——先扫后投。
