...
**状态**：D22（revision 7 body），R6 P1 已修 + terra 两轮扫描全吸收（R36：root→T-A、response 面→T-H；R37：collision 从 respond 改判 **T-I close**——echo 会被误当 in-flight 终态响应，correlation-trust 失败）——pending 复扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review（不 route Terra pre-verdict）。
...
- [x] **every pre-dispatch rejection class has exactly one deterministic disposition** — your R6 invariant adopted verbatim; disposition table T-A…T-I over all eight rejection surfaces, including non-object frame roots (T-A), the inbound response/callback surface (T-H), and in-flight id collisions as their own close-class (T-I — echoing under a colliding id would break correlation trust); strict-profile id route: detectable-but-profile-invalid → close, no response; close-classes excluded from the public union/proofs by construction
