...
**状态**：D31（revision 10 body），R9 P1 已修 + terra 两轮扫描全吸收（R45：envelope 全传播 + T-D totality；R46：response lane 拆 accept/close——valid correlated → T-L 静默 settle，其余 → T-H close；jsonrpc-const fixture）——pending 复扫——先扫后投。
