...
**状态**：D29（revision 10 body），R9 P1 已修（response-candidate precedence：无 method 且带 result/error 的帧一律 T-H close，先于 Request/Notification/ID 路由；T-D 收窄；method-bearing extras → T-F/T-K）——pending 窄扫——先扫后投 + terra R39/D24 扫描三 finding 已吸收（残留段删除 / T-K 扩至 idless envelope-shape 违规 / gates 时态）（T-D 三分：结构非法 idless → null；合法 Notification → T-J accept / T-K close，§4.1 MUST-NOT-reply 逐字采纳；表改为六维 state partition 投影）——pending 窄扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review。
...
**State partition (absorbs your R7 and R9 audits — the table is the projection of this product, not a list of patched examples):** outcome derives from ① frame/parse canonicality (T-A/T-B/T-C gate first), ② **response-candidate precedence (R44, maintainer R9): an object with no `method` and at least one of `result`/`error` is a response candidate and routes T-H before any Request/Notification or ID-profile routing** — regardless of missing/invalid id, malformed members, `result`+`error` together, or failed correlation; a **method-bearing** object stays on the Request/Notification lane, where an extra `result`/`error` member is an envelope-shape violation (T-F for a profile-valid-id Request, T-K for an idless Notification), ③ **Request-object structural validity** (`jsonrpc` const + `method` string present), ④ **ID state** (absent | profile-valid | detectable-profile-invalid | ambiguous/untrusted), ⑤ **notification eligibility and method direction**, ⑥ invocation/schema validity. A canonical **idless non-response-shaped** object splits by ③: structurally valid ⇒ it **is a JSON-RPC Notification (§4.1)**; structurally invalid ⇒ it is not a Request object at all and no ID is available. Your rule is adopted verbatim: **once a canonical object is classified as a Notification, no JSON-RPC Response is emitted for any downstream method/params/profile rejection.**
