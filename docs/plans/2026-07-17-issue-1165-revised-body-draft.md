...
**状态**：D26（revision 8 body），R7 P1 已修 + 六案例计数同步（FC-R40-1） + terra R39/D24 扫描三 finding 已吸收（残留段删除 / T-K 扩至 idless envelope-shape 违规 / gates 时态）（T-D 三分：结构非法 idless → null；合法 Notification → T-J accept / T-K close，§4.1 MUST-NOT-reply 逐字采纳；表改为六维 state partition 投影）——pending 窄扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review。
