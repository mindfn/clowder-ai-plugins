...
**状态**：D21（revision 7 body），R6 P1 已修 + terra R35/D20 扫描五 finding 已吸收（table 补全 T-A…T-H：collision→T-F、非 object root→T-A、response/callback 面→T-H）——pending 复扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review（不 route Terra pre-verdict）。
...
## Status and request — revision 7: your R6 exact-body audit absorbed — every pre-dispatch rejection class now has exactly one deterministic disposition
...
Your R6 exact-body audit ([5015117839](https://github.com/zts212653/clowder-ai/issues/1165#issuecomment-5015117839)) found the remaining P1: a **detectable but profile-invalid request id** (numeric, `null`, non-string, over-length, or grammar-invalid) had no deterministic wire disposition — JSON-RPC 2.0 §§4–5 forbids echoing it into the narrower profile and forbids `null` once detection succeeded — and the same response-or-close ambiguity spanned the sibling W0/W2 pre-dispatch classes. **Fixed in this revision via your recommended strict-profile route:** your governing invariant is adopted verbatim and stated once, a **pre-dispatch disposition table (T-A…T-H)** assigns every raw-frame/profile/request/response rejection class exactly one outcome — closed byte-proved JSON-RPC error with class-valid id semantics, or connection close with no response — the Invalid-Request no-usable-id arm narrows to **id-absent**, close-classes are excluded from the public error union and byte proofs by construction (the eleven declared variants are unchanged), and the disposition conformance set proves zero dispatch and zero response bytes before every close.
...
- **`WireNotification`** = same shape **without `id`** — legal only for registry rows declared as notifications (row 10 `host.grants.changed`); a notification carrying an `id`, or a request without one, takes its disposition-table outcome — **T-F (`Invalid Request`, id echoed) and T-D (`Invalid Request`, `id: null`) respectively** — not an ad-hoc connection-level rule.
...
| T-D | `id` member **absent** from a request position | envelope | **respond** `Invalid Request` (−32600), `id: null` — the no-usable-id arm, narrowed to exactly this class |
...
| T-F | canonical frame, profile-valid id, envelope/state violation: unknown/direction-invalid `method`, missing/extra envelope members, a notification carrying an `id`, `params` not matching the closed two-key form, **or a profile-valid id colliding with an in-flight request in the same `brokerSessionId`** | envelope schema + in-flight state | **respond** the matching standard error (`Invalid Request` / `Method not found`), **id echoed byte-for-byte** |
| T-G | canonical frame, profile-valid id, valid envelope, parsed-**value** violation inside `params` (schema mismatch, `deadlineUnixMs` out of range, row-input violations) | schema/value validation | **respond** `Invalid params` or the row's closed application error, **id echoed byte-for-byte** |
| T-H | inbound **response/callback-ack** validation failure: a `WireSuccessResponse`/`WireErrorResponse` that is schema-invalid or whose `id` correlates to no in-flight outbound request; a row-9 delivery ack whose `deliveryId` fails byte-equality | response correlation/schema | **close, no response** — JSON-RPC defines no response to a response; settlement/lease/watermark state moves zero steps (the row-9 delivery stays unsettled under your K-2 retry/dead-letter policy) |

The lexeme/value split is load-bearing: a non-canonical **raw lexeme** anywhere is T-C (close), while a canonical lexeme whose parsed **value** violates its profile is T-G (respond) — except the id position itself, where a detected value outside the id profile is T-E (close). **The table is exhaustive by construction over the seven rejection surfaces: frame bytes/root form (T-A), JSON parse (T-B), canonical-token identity (T-C), the id position (T-D/T-E), request envelope + in-flight id state (T-F), `params` values (T-G), and the inbound response/callback surface (T-H); a new rejection surface cannot be introduced without extending this table in the same shape delta.** The W0/W1/W2/W3 rows above carry their class references. Disposition conformance adds: numeric / `null` / non-string / over-length / grammar-invalid id, duplicated `id` member, non-canonical id spelling, **scalar/array frame root**, the sibling W0/W2 lexeme failures, **the in-flight collision (respond, echo)**, and **T-H cases (mis-correlated `WireSuccessResponse`; row-9 delivery-ack `deliveryId` mismatch — close with zero settlement/lease/watermark movement)** — each close-class case proving zero dispatch and zero response bytes before close, each respond-class case proving the exact variant and id arm; production close behavior itself is yours (K-2), exercised at the joint gate.
...
| Invalid Request — no-usable-id arm | `-32600` | `"Invalid Request"` | `null` (id absent from a request position — the sole no-usable-id class; other historical readings are re-assigned by the disposition table: T-C/T-E close with no response) | absent |
...
- [x] **every pre-dispatch rejection class has exactly one deterministic disposition** — your R6 invariant adopted verbatim; disposition table T-A…T-H over all seven rejection surfaces, including in-flight id collision (T-F), non-object frame roots (T-A), and the inbound response/callback surface (T-H); strict-profile id route: detectable-but-profile-invalid → close, no response; close-classes excluded from the public union/proofs by construction
...
- [ ] **fresh review on revision-7 exact live bytes** (raw API-string SHA-256/counts reported in delivery comment)
