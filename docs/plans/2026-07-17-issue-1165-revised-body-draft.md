...
**状态**：D24（revision 8 body），R7 P1 已修（T-D 三分：结构非法 idless → null；合法 Notification → T-J accept / T-K close，§4.1 MUST-NOT-reply 逐字采纳；表改为六维 state partition 投影）——pending 窄扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review。
...
## Status and request — revision 8: your R7 exact-body audit absorbed — the disposition table is now the projection of the six-way state partition, and no Notification ever receives a Response
...
- **`WireNotification`** = same shape **without `id`** — legal only for registry rows declared as notifications (row 10 `host.grants.changed`). An object carrying an `id` is a Request (a notification-only method with an `id` is T-F: `Invalid Request`, id echoed). A canonical **idless** object splits per §4.1 (your R7 audit): structurally valid ⇒ it **is a Notification** and never receives a Response — legal row-10 → **T-J accept**; request-only/unknown/direction-invalid method or invalid params → **T-K close, no response, zero dispatch**; structurally invalid (missing/invalid `jsonrpc`/`method`) ⇒ not a Request object at all → **T-D (`Invalid Request`, `id: null`)**.
...
**State partition (absorbs your R7 audit — the table is the projection of this six-way product, not a list of patched examples):** outcome derives from ① frame/parse canonicality (T-A/T-B/T-C gate first), ② **Request-object structural validity** (`jsonrpc` const + `method` string present), ③ **ID state** (absent | profile-valid | detectable-profile-invalid | ambiguous/untrusted), ④ **notification eligibility and method direction**, ⑤ invocation/schema validity, ⑥ inbound request versus inbound response. A canonical **idless** object splits by ②: structurally valid ⇒ it **is a JSON-RPC Notification (§4.1)**; structurally invalid ⇒ it is not a Request object at all and no ID is available. Your rule is adopted verbatim: **once a canonical object is classified as a Notification, no JSON-RPC Response is emitted for any downstream method/params/profile rejection.**
...
| T-D | canonical idless object that is **not a valid JSON-RPC Request object at all** (missing/invalid `jsonrpc` or `method`) — no ID is available and the object is not a Notification | Request-object structural validity | **respond** `Invalid Request` (−32600), `id: null` — the no-usable-id arm, narrowed to exactly this true-detection-failure class |
...
| Invalid Request — no-usable-id arm | `-32600` | `"Invalid Request"` | `null` (a canonical idless object that is not a valid Request object at all — the sole no-usable-id class; a structurally valid idless object is a Notification → T-J/T-K, never a Response; other historical readings are T-C/T-E close) | absent |
...
- [ ] **fresh review on revision-8 exact live bytes** (raw API-string SHA-256/counts reported in delivery comment)
