...
**状态**：D23（revision 7 body），R6 P1 已修 + terra 三轮扫描全吸收（R36：root→T-A、response 面→T-H；R37：collision 改判 **T-I close**；R38：W3 close-class 投影补全五类）——pending 复扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review（不 route Terra pre-verdict）。
