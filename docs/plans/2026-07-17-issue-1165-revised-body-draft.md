...
**状态**：D20（revision 7 body），R6 P1 已修（pre-dispatch disposition table + strict-profile id route）——pending 本 thread 双猫窄扫（sol/terra finding-only）——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review（不 route Terra pre-verdict）。
...
| W1 | rows 1–9, 11–12 request/response/error `id` | ASCII `RequestId` | 1..128; `^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$`; in-flight unique/session | `ASCII(128)` = 3..130 encoded bytes; response echo byte-equal; **a detected id outside this profile → T-E close, no response** | T4 exact owner value | **CLOSED** |
...
|---|---|---|---|
| T-A | transport/framing byte failure: invalid UTF-8, BOM, blank frame, batch array, trailing non-whitespace, unterminated frame over `maxFrameBytes` | W0 streaming | **close, no response** (your canonical ceiling rule, uniform for the class) |
...
- [x] **every pre-dispatch rejection class has exactly one deterministic disposition** — your R6 invariant adopted verbatim; disposition table T-A…T-G (strict-profile id route: detectable-but-profile-invalid → close, no response); close-classes excluded from the public union/proofs by construction
