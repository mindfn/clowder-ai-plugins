...
**状态**：D30（revision 10 body），R9 P1 已修（response-candidate precedence 为 partition 步骤②）+ terra R44/D29 扫描三 finding 已吸收（envelope prose 全传播 / T-D 改为 non-response-candidate 语义恢复 totality / 本状态行合并）——pending 窄扫——先扫后投。
