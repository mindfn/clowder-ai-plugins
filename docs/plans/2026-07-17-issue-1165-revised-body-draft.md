...
**状态**：D28（revision 9 body），R8 P1 已修 + terra R42/D27 扫描两 finding 已吸收（gates post-R8 / W2 加粗重建） + terra R39/D24 扫描三 finding 已吸收（残留段删除 / T-K 扩至 idless envelope-shape 违规 / gates 时态）（T-D 三分：结构非法 idless → null；合法 Notification → T-J accept / T-K close，§4.1 MUST-NOT-reply 逐字采纳；表改为六维 state partition 投影）——pending 窄扫——**先扫后投**；投递报新 raw API-string hash/count packet，maintainer exact-body re-review。
...
## Status and request — revision 9: your R8 exact-body audit absorbed — parsed-value routing is now itself a projection of ID/Notification state; no projection routes a Notification to T-G
...
- [ ] **fresh review on revision-9 exact live bytes** (raw API-string SHA-256/counts reported in delivery comment)
