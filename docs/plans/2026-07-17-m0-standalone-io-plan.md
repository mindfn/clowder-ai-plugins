...
> **Status: draft R9 — field-level co-signable shape everywhere except the snapshot sub-protocol, which is deliberately restructured into a ★★ DESIGN PACKET (fixed invariants + three owner decision rows + conditional candidate shapes) after five review rounds proved its free variables live in K-1/K-2 ground truth. Row 8 may detach into its own decision round; rows 1–7 & 9–12 plus handshake/framing/digest/settlement/error-mapping are complete. Registry deltas from the canonical matrix are all ★-disclosed (none silent). Absorbed the K-2 maintainer verdict on [clowder-ai#1165](https://github.com/zts212653/clowder-ai/issues/1165) (comment 2026-07-17T06:11Z; canonical assessment `main@b32170a8`). Pending explicit `shape-approved` on the revised issue body. No implementation, no beta.3 publication, no K-1/K-2 re-pin before that reply.**
...
- Pages bounded: `maxItems` 1..64, `pageByteBudget = 786_432`, item ceiling `maxSerializedItemBytes = 393_216`; oversize stored item = system fault (`SNAPSHOT_UNAVAILABLE`), never caller `VALIDATION`, never silent skip.
