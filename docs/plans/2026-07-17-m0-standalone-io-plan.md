...
> **Status: draft R11 — PENDING on four open gates (three snapshot decision rows · row-6 read paging · registry-wide generated byte proofs · #1168 canonical wording), per the maintainer P1 addendum ([#1165 comment 5000048510](https://github.com/zts212653/clowder-ai/issues/1165#issuecomment-5000048510)). All byte ceilings are provisional candidates pending generated proofs; the byte-proof gate has explicit three-layer execution ownership (P-1a generates proofs/validators · P-1b harness executes oracles · K-2 owns production enforcement at the joint gate). Complete-and-stable: handshake, framing, digest, settlement, wire-error mapping, registry rows 1–5/7/10–12, DeliveryRejection, K-1 truth, release sequence. Snapshot stays a ★★ DESIGN PACKET; registry deltas all ★-disclosed. Whether approval splits (rows 6/8 detaching) is the maintainer's call. No #1165 body update until #1168 merges and pins canonical wording. No implementation, no beta.3, no re-pin.**
...
**★ Registry-wide byte-proof invariant (maintainer addendum, FC-24):** every published row carries **generated** `maxEncodedRequestBytes` / `maxEncodedResultBytes` / `maxEncodedErrorBytes` derived from its schemas (ASCII, multibyte UTF-8, and JSON-escaping worst cases) — hand-computed budgets do not qualify; a row without proofs below the frame ceiling cannot publish. Enforcement is three-stage and fail-closed: **pre-dispatch** (validate request against row proof before business dispatch) → **pre-state** (a dynamically assembled page is fully composed before any watermark/lease/settlement advances) → **pre-write** (final frame re-validated before the write-queue mutation). Conformance adds: max-boundary frames in all three encodings, +1-byte oversize rejection, pagination continuation, and zero-side-effect oversize rejection.
...
4. **#1168 canonical wording**: the addendum's canonical fix PR is still open (`2f459e5` at scan time) — final wording/SHA is pinned only after it merges.
