...
**State (rewritten R10 per FC-25; cycle resolved R14): PENDING on two shape gates (gate 4 closed in R12; gate 3 relocated in R14 — see below).**
...
3. **Generated byte proofs — OWNER DECISION REQUIRED (R15, FC-33; R14's unilateral relocation withdrawn):** the maintainer's [#1165 P1 addendum](https://github.com/zts212653/clowder-ai/issues/1165#issuecomment-5000048510) states verbatim that *“`shape-approved` remains withheld and the required revision now also must”* give every ready method a generated request/result/error proof — i.e. proofs are written into the **current shape-approved withheld condition**, not deferred to publication. R14 read the merged doc's *method-level* reserved-vs-ready rule (a reserved method is not publish-ready) and wrongly used it to override the *issue-level* phase gate; that was an unauthorized re-sequencing of an owner decision. Both one-hand rules hold simultaneously and neither implies proofs may follow approval. The lifecycle order is therefore a maintainer decision, surfaced as the first Decision Row of the #1165 body — **A:** reservation-only shape co-sign, proofs deferred to P-1a publication readiness; **B:** proofs stay a shape gate, maintainer authorizes a pre-shape proof-spike (proof metadata + validators only) before `shape-approved`. This plan states no preference and presumes no relocation until the maintainer answers;
