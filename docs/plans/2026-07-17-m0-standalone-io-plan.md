...
> **Status: draft R12 — #1168 merged (`d606aab`); canonical `wireBounds` wording pinned verbatim (v0 compact encoding profile · four-condition publication invariant · maxLength-is-not-a-byte-proof ruling · proof-coverage list · reserved-vs-ready rule). Remaining open gates: three snapshot decision rows + row-6 read paging schemas (both owner-decided). Byte ceilings stay provisional pending generated proofs; three-layer proof ownership unchanged (P-1a generates · P-1b executes · K-2 enforces). Complete-and-stable: handshake, framing incl. encoding profile, digest, settlement, wire-error mapping, rows 1–5/7/10–12, DeliveryRejection, K-1 truth, release sequence. Snapshot stays a ★★ DESIGN PACKET. **Ready for #1165 body update** requesting: stable-portion co-sign + three snapshot decision rows + read/byte-proof corrections confirmation; approval split is the maintainer's call. No implementation, no beta.3, no re-pin.**
...
**State (rewritten R10 per FC-25 — the R6 "every precondition satisfied" claim is withdrawn as self-contradictory with the design packet): PENDING on four explicitly open gates.**
...
4. ~~#1168 canonical wording~~ — **CLOSED (R12)**: #1168 merged as `d606aab859883422b04d996cf223560fc20ae232` (head `2f459e5` unchanged since our R10 anchor — no post-anchor commits); the `wireBounds` truth, v0 compact encoding profile, four-condition publication invariant, proof-coverage field list, and reserved-vs-ready rule are pinned verbatim into this plan.
