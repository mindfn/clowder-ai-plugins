...
> **Status: draft R13 — #1168 merged (`d606aab`); canonical `wireBounds` wording semantically pinned (equivalently restated, provenance = `d606aab`) (v0 compact encoding profile · four-condition publication invariant · maxLength-is-not-a-byte-proof ruling · proof-coverage list · reserved-vs-ready rule). Remaining **shape** gates — **two**: ① three snapshot decision rows · ② row-6 read paging schemas (both owner-decided). Generated byte proofs are **not** a shape gate (FC-32 dependency-cycle fix): per the canonical reserved-vs-ready rule a method may be *reserved* without proof but cannot *publish* without it — proofs are therefore P-1a's **publication readiness gate**, produced after `shape-approved`, with any proof failure returning to #1165 as a delta. Byte ceilings stay provisional pending generated proofs; three-layer proof ownership unchanged (P-1a generates · P-1b executes · K-2 enforces). Complete-and-stable: handshake, framing incl. encoding profile, digest, settlement, wire-error mapping, rows 1–5/7/10–12, DeliveryRejection, K-1 truth, release sequence. Snapshot stays a ★★ DESIGN PACKET. **Ready for #1165 body update** requesting: stable-portion co-sign + three snapshot decision rows + read/byte-proof corrections confirmation; approval split is the maintainer's call. No implementation, no beta.3, no re-pin.**
...
3. ~~Global generated byte proofs~~ — **RELOCATED (R14, FC-32)**: listing this as a shape gate created the cycle `shape-approved → P-1a → proofs → shape-approved` (proofs are generated by P-1a, which runs only after approval). Canonical resolution needs no new maintainer decision — the merged reserved-vs-ready rule already layers it: reservation needs no proof, **publication** does. Proofs are P-1a's publication readiness gate; a row whose generated proof fails returns to #1165 as a delta request. Hand-computed budgets stay demoted to *pending generated proof*;
